 ...
 */
public class KcutAlgorithm implements IPeterDeploy {
...
//							System.out.println(cantuseVm);
				...
				if(!cantuseVm.contains(nodeID)&&nodeID!=-1)
			...
			if (node.getTotalComputationCost() <= averageVMLoad) {
				for (PeterService service : undeployedService) {
					...
					for (TaskLink serviceLink : service.getGlobalServicePool()) {
						...
						System.err.println(service.getId() + "->" + ((PeterService)serviceLink.getNextTask()).getId() + ":" + serviceLink.getWeight());
...
//					System.out.println("Diff( " + (service.getId()) + " , " + i + " ) = " + service.getCostDiff(i));
					
					/**
					 * end
					 */
					
					if (service.getCostDiff(i) <= minimalValue && node.getTotalComputationCost() + service.getComputationCostByRate() <= averageLoad) {
