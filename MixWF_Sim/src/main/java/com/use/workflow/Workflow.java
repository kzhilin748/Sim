...
public class Workflow implements IWorkflow{
	...
	private int Ta;			// Total(tasknp*task runtime)/all np 
	private int Timeofcp;	//cplist run time
	private int NPnumber =3;
	...
	public void setTaskOneCPURunTime(){
		List<IDepend>tasklist=this.getTaskList();
		for(IDepend tmp :tasklist){
			DAGDependTask task=(DAGDependTask) tmp;
			task.setOneCPUcomputationTime(task.getComputationTime());
		}
	}
	...
	public List<IDepend> getCriticalPathTasksList() {
		...
		while(criticalPathTask!=null) {
			...
			if(criticalPathTask.getChildTaskLink().isEmpty()) {
				...
				int MinTopRK=Integer.MAX_VALUE;
				for(TaskLink temp2 : criticalPathTask.getChildTaskLink()) {
					DAGDependTask child = (DAGDependTask)temp2.getNextTask();
					if(child.getBottomRank()+child.getTopRank()==criticalPathRankValue&&child.getTopRank()<MinTopRK) {
						MinTopRK=child.getTopRank();
	...
	public List<IDepend> getNowTimeCPList(){
		List<IDepend> CPLIST =new ArrayList<IDepend>();
		DAGDependTask task = (DAGDependTask) taskList.get(0);
		int CPRank =task.getTopRank()+task.getBottomRank();
		for(IDepend tmp : taskList){
			DAGDependTask _task = (DAGDependTask)tmp;
			if((_task.getTopRank()+_task.getBottomRank())==CPRank){
				CPLIST.add(_task);
			}
		}
		return CPLIST;
	...
	public void ReTopandBottomRank(){
		this.topRanking();
		this.bottomRanking();
	}
