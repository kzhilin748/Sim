...
public class MLSQsortBybottomRK {
...
//				System.out.println(DAGMLSTime);
	...
	private static long findbestGAP(List<Long> Timerecord,List<Integer> cpuRecord,DAGDependTask task){
		...
		for(int i =0;i<RecordSize;i++){
			...
			for(int j =i+1;j<RecordSize;j++){
				...
				if(EndTime-beginTime<task.getComputationTime()&&thisTimeCpu>=task.getNumberOfProcessors())
					continue;
	...
	private static List<IDepend> sortBybottomRK(List<IDepend> taskList){
		...
		Collections.sort( taskList, new Comparator<IDepend>(){
		    public int compare( IDepend task1, IDepend task2 )
		    {
				...
				return -(task.getBottomRank()-_task.getBottomRank());
		    }
		});
